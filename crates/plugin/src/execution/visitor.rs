...
pub(crate) fn value_event<'a>(code: i32, payload: Payload<'a>) -> ValueEvent<'a> {
    match (EventCode::from_raw(code), payload) {
        ...
        (Some(EventCode::SelectionSet), Payload::Nil) => ValueEvent::SelectionSet(None),
        (Some(EventCode::VlaObject), Payload::Nil) => ValueEvent::VlaObject(None),
        (Some(EventCode::File), Payload::Nil) => ValueEvent::File,
        (Some(EventCode::Function), Payload::Nil) => ValueEvent::Function(None),
